...
export function detectPlatform(): PlatformInfo {
  ...
  return { os: osPlat as 'darwin' | 'linux', arch, libc };
}
 ...
 */
export function makeAssetName(version: string, platform: PlatformInfo): string {
  const { os: osName, arch } = platform;
  
  const plat =
    osName === 'darwin' ? `darwin-${arch}` :
 ...
 */
export function getExpectedAssetName(version: string = 'v0.1.0'): string {
